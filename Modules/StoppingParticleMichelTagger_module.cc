...
void StoppingParticleMichelTagger::produce(art::Event & evt) {
   ...
   if (selection_v.at(0)->GetSelectionStatus()){
      ...
      */
      ...
      // We actually want the map the other way around to the above so that we can loop over pfps...
      // Copying over code from FillTree (should be cleaned up a little)
      ...
      // Loop through PFPs. For each track we want to run the various HitCosmicTag algorithms and store the output.
      for (auto pfp_i : pfps) {

         // Define default values (for non-tracks)
         double length = -9999;
         double deltax = -9999;
         ...
         int sc_tosave_start_index = -9999;
         std::vector<double> sc_tosave_dqds_v = {};
         std::vector<double> sc_tosave_ds_v = {};
         std::vector<double> sc_tosave_dqds_slider = {};
         std::vector<double> sc_tosave_linearity_v = {};
         bool passed = false;
         bool ct_result_michel = false;
         bool ct_result_bragg = false;

         // Find track associated to each PFP (if any)
         std::vector<art::Ptr<recob::Track>> tracks_pfp = tracks_from_pfps.at(pfp_i.key());
         if(tracks_pfp.size() > 1) {
            mf::LogError(__PRETTY_FUNCTION__) << "PFP associated to more than one track." << std::endl;
            throw std::exception();
         }

         //"Loop" over tracks. Should only ever be 0 or 1, so basically the same as "if there's a track:"
         for (auto track_i : tracks_pfp){
            ...
            deltax = track_i->Vertex().Z() - track_i->End().Z();
            ...
            // Find clusters first...
            auto iter2 = pfps_to_clusters.find(pfp_i);
            ...
            sc_tosave_start_index = sc_tosave._start_index;
            sc_tosave_dqds_v = sc_tosave._dqds_v;
            sc_tosave_ds_v = sc_tosave._ds_v;
            sc_tosave_dqds_slider = sc_tosave._dqds_slider;
            sc_tosave_linearity_v = sc_tosave._linearity_v;

            length = track_i->Length();

         } //End loop over recob::Tracks

         TPCObj_PFP_track_length.emplace_back(length);
      ...
      } // End loop over recob::PFParticles
