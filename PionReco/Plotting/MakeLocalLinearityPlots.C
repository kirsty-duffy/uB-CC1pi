...
void PlotLocalLinearityDetails(std::vector<std::vector<double>> spacepoints_vec, std::vector<double> proj1d_vec, std::vector<double> cusum_vec, std::vector<double> ttest_vec, std::vector<int> kinkidxs_vec, int pfp_idx, std::vector<double> startpoint_vec, std::vector<std::vector<double>> truekink_vec=std::vector<std::vector<double>>(), std::vector<std::vector<double>> mcp_x_vec=std::vector<std::vector<double>>(), std::vector<std::vector<double>> mcp_y_vec=std::vector<std::vector<double>>(), std::vector<std::vector<double>> mcp_z_vec=std::vector<std::vector<double>>(), double mincosth=-999, int evtnum=-999){
  ...
  for (int i_true=0; i_true<truekink_vec.size(); i_true++){
    ...
    truekink_yz->SetPoint(i_true,z,y);
  ...
  c1->Print(TString::Format("evt_%d_pfp_%d.pdf",evtnum,pfp_idx).Data());
...
void PlotLocalLinearityDetails_byangle(std::vector<std::vector<double>> spacepoints_vec, std::vector<double> locallin_vec, std::vector<int> kinkidxs_vec, int pfp_idx, std::vector<double> startpoint_vec, std::vector<std::vector<double>> truekink_vec=std::vector<std::vector<double>>(), double mincosth=-999, int evtnum=-999){
  ...
  // std::cout << "Start point: " << "(" << startpoint_vec.at(0) << ", " << startpoint_vec.at(1) << ", " << startpoint_vec.at(2) << ")" << std::endl;
  startpoint_yz->SetPoint(0,startpoint_vec.at(2),startpoint_vec.at(1));
  startpoint_xz->SetPoint(0,startpoint_vec.at(2),startpoint_vec.at(0));

  // Fill local linearity plots
  for (size_t i_pt=0; i_pt < locallin_vec.size(); i_pt++){
    locallin->SetPoint(i_pt,i_pt,locallin_vec.at(i_pt));
  }
  ...
  for (int i_true=0; i_true<truekink_vec.size(); i_true++){
    ...
    double y = truekink_vec.at(i_true).at(1);
    double z = truekink_vec.at(i_true).at(2);
    std::cout << "true kink: " << "(" << x << "," << y << "," << z << ")" << std::endl;
    truekink_yz->SetPoint(i_true,z,y);
    truekink_xz->SetPoint(i_true,z,x);
  }
  ...
  // Draw local linearity plot
  c1->cd(3);
  locallin->GetXaxis()->SetTitle("Spacepoint index in PFP (coloured PFP only)");
  locallin->GetYaxis()->SetTitle("Local linearity");
  // locallin->GetYaxis()->SetRangeUser(0,1.05);
  locallin->Draw("APL");
  if (breakpoints_idx->GetN()>0) breakpoints_idx->Draw("P same");
