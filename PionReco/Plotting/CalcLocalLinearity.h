...
std::vector<double> GetLocalLinearityVec(std::vector<std::vector<double>> spacepoints, int _slider_window){
  ...
  for (size_t i_pos=0; i_pos<spacepoints.size(); i_pos++){
    ...
    // If there aren't enough hits after this hit in the vector, return 1 because we can't get a good measurement
      ...
      for (size_t i_w=i_pos-n_either_side; i_w<i_pos; i_w++){
        ...
        TVector3 vec0(spacepoints.at(i_w).at(0),spacepoints.at(i_w).at(1),spacepoints.at(i_w).at(2));
        TVector3 vec1(spacepoints.at(i_w+1).at(0),spacepoints.at(i_w+1).at(1),spacepoints.at(i_w+1).at(2));
...
std::vector<std::vector<double>> OrderSpacepoints(std::vector<std::vector<double>> spacepoints, std::vector<double> startpoint){
  ...
  ordered_spacepoints.push_back(spacepoints.at(firstpoint_idx));
  ...
  while (used_indices.size() < spacepoints.size()){
    ...
    std::vector<double> curr_sp = ordered_spacepoints.at(ordered_spacepoints.size()-1);
    ...
    ordered_spacepoints.push_back({nextpoint.X(),nextpoint.Y(),nextpoint.Z()});
